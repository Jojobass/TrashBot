...
async def check_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
	...
	if user_info[4] != READY:
		...
		await update.message.reply_html(
			...
			f'{user_info[3]}',
			reply_markup=ReplyKeyboardMarkup(
				[["Редактировать имя", "Редактировать адрес"], ["Редактировать номер", "Редактировать комментарий"],
				 ["Оформить заказ"]],
...
async def unknown(update: Update, context: ContextTypes.DEFAULT_TYPE):
	await context.bot.send_message(chat_id=update.effective_chat.id, text="Сорри, я не знаю таких команд.")
...
def get_user_info(chat_id):
	cur = conn.cursor()
	cur.execute(f"SELECT name, address, phone, comment, state FROM user_info WHERE chat_id = {chat_id};")
...
if __name__ == '__main__':
	...
	check_details_handler = MessageHandler(filters.Text(['Вынести мусор']) | filters.Text(['Детали заказа']),
	                                       check_details)
	text_handler = MessageHandler(filters.TEXT & ~(
			filters.Text(['Вынести мусор']) | filters.Text(["Редактировать комментарий"]) |
